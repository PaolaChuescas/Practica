...
##MODIFICADO
EDA_kde_oxid = uic.loadUi("Ui/EDA_kernel_oxid.ui")
...
def getFile_EDA_oxid():
    ...
    EDA_oxid.filename=QFileDialog.getOpenFileName(filter="xlsx file (*.xlsx)")[0]
    EDA_oxid.archivo=EDA_oxid.filename.split('/')
   # EDA_oxid.archivo=EDA_oxid.filename
...
##MODIFICADO
    EDA_oxid.xlsxfile = EDA_oxid.filename
    try:
        EDA_oxid.df =pd.read_excel(EDA_oxid.xlsxfile)
        print(EDA_oxid.df)  
    except Exception as e:
        print(e)

    EDA_oxid.prenombre = str(EDA_oxid.xlsxfile).split("/")
    EDA_oxid.nombre = EDA_oxid.prenombre[-1]
    
    EDA_oxid.nfilas = EDA_oxid.df.shape[0]
    EDA_oxid.ncolumnas = EDA_oxid.df.shape[1]
    print(EDA_oxid.df.columns)
...
def EDA_verbase_oxid():
    ...
    EDA_base_oxid.xlsxfile = EDA_oxid.filename
    try:
        EDA_base_oxid.df =pd.read_excel(EDA_base_oxid.xlsxfile)
    except Exception as e:
        print(e)

    buffer = io.StringIO()
    
    EDA_base_oxid.df.info(buf=buffer)
    EDA_base_oxid.lines = buffer.getvalue().splitlines()
    
    #Bien
    EDA_base_oxid.database2 = EDA_base_oxid.df
    EDA_base_oxid.model_data = PandasModel(EDA_base_oxid.database2)
    #EDA_base_oxid.model_data = EDA_base_oxid.df
...
def EDA_verscatter_oxid():
    #print(value1)
    EDA_base_oxid.df = EDA_oxid.filename
    try:
        EDA_scatter_oxid.df =pd.read_excel(EDA_oxid.xlsxfile)
    except Exception as e:
        print(e)
    ...
    print(EDA_scatter_oxid.df)
    x_vars = list(EDA_scatter_oxid.df.columns)
    i=0
     
    while i < (len(x_vars)):
        y_vars = x_vars[i]
        g = sns.PairGrid(EDA_scatter_oxid.df, x_vars=x_vars, y_vars=y_vars)
...
def EDA_verkde_oxid():
     #print(value1)
     EDA_kde_oxid.xlsxfile = EDA_oxid.filename
     try:
         EDA_kde_oxid.df =pd.read_excel(EDA_kde_oxid.xlsxfile)
     except Exception as e:
         print(e)
     ...
     try:
         EDA_kde_oxid.horizontalLayout.removeWidget(EDA_kde_oxid.toolbar)
         EDA_kde_oxid.verticalLayout.removeWidget(EDA_kde_oxid.canv)
         sip.delete(EDA_kde_oxid.toolbar)
         sip.delete(EDA_kde_oxid.canv)
         EDA_kde_oxid.toolbar=None
         EDA_kde_oxid.canv=None
         EDA_kde_oxid.verticalLayout.removeItem(EDA_kde_oxid.spacerItem1)
     except Exception as e:
         print(e)
         pass
     
     EDA_kde_oxid.canv=MatplotlibCanvas(EDA_kde_oxid)
     EDA_kde_oxid.toolbar=Navi(EDA_kde_oxid.canv,EDA_kde_oxid.centralwidget)
     EDA_kde_oxid.horizontalLayout.addWidget(EDA_kde_oxid.toolbar)
     EDA_kde_oxid.verticalLayout.addWidget(EDA_kde_oxid.canv)
     EDA_kde_oxid.canv.axes.cla()
     ax=EDA_kde_oxid.canv.axes
     ...
     x_vars=list(EDA_kde_oxid.df.columns)
     i=0
     print(len(list(EDA_kde_oxid.df.columns)))
     
     while i < (len(list(EDA_kde_oxid.df.columns))):
          y_vars = x_vars[i]
          g = sns.PairGrid(EDA_kde_oxid.df, x_vars= x_vars,y_vars=y_vars)
       #   g.map(sns.scatterplot, s=15)
          g.map(sns.kdeplot, lw=2)
          legend=ax.legend()
          legend.set_draggable(True)
          i=i+1        
...
def EDA_verplotcorr_oxid():
     #print(value1)
     EDA_plotcorr_oxid.xlsxfile = EDA_oxid.filename
     try:
         EDA_plotcorr_oxid.df =pd.read_excel(EDA_plotcorr_oxid.xlsxfile)
     except Exception as e:
         print(e)
     ...
     x_vars=list(EDA_plotcorr_oxid.df.columns)
     print(x_vars)
     i=0
     ...
     while i < (len(x_vars)):
         y_vars = x_vars[i]
         print(y_vars)
         corr = EDA_plotcorr_oxid.df.set_index(y_vars).corr()
...
#MODIFICADO
EDA_oxid.button_kde.clicked.connect(EDA_oxid_kde)
...
#MODIFICADO
#botones EDA_oxid_kernel
EDA_kde_oxid.button_main.clicked.connect(kde_EDA_oxid)
